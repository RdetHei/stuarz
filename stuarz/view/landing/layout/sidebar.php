...
<div id="sidebar" class="fixed inset-y-0 left-0 w-64 bg-[#0f172a] text-white flex flex-col transition-all duration-300 z-50">
  ...
  <div class="relative flex items-center justify-center h-16 border-b border-gray-700">
    ...
    <button id="sidebarToggle" class="absolute right-2 text-white focus:outline-none" aria-label="Toggle sidebar internal">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-8 block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
      </svg>
    </button>
  </div>
  ...
  <nav class="flex-1 px-2 py-4 space-y-2">
    ...
    <!-- Tombol Kelas (hover untuk menampilkan popup) -->
    <div class="relative" id="kelasWrapper">
      <button id="kelasBtn"
        class="flex items-center px-3 py-2 text-sm rounded-lg hover:bg-gray-700 w-full focus:outline-none"
        aria-haspopup="true"
        aria-expanded="false"
        aria-controls="kelasPopup">
        <span class="material-symbols-outlined mr-3">school</span>
        <span class="menu-text">Kelas</span>
        <!-- Icon di akhir teks untuk menandakan popup -->
        <span class="material-symbols-outlined ml-auto">chevron_right</span>
      </button>
    </div>

    <!-- Popup akan dipindahkan ke body lewat JS -->
    <div id="kelasPopup"
      class="hidden fixed bg-white text-gray-800 rounded-xl shadow-lg w-48 p-3 z-[9999] pointer-events-auto"
      role="menu"
      aria-hidden="true">
      <a href="index.php?page=tugas"
        class="flex items-center p-2 rounded-lg hover:bg-gray-100" role="menuitem">
        <span class="material-symbols-outlined text-blue-600 mr-2">assignment</span> Tugas
      </a>
      <a href="index.php?page=absensi"
        class="flex items-center p-2 rounded-lg hover:bg-gray-100" role="menuitem">
        <span class="material-symbols-outlined text-green-600 mr-2">event_available</span> Absensi
      </a>
      <a href="index.php?page=jadwal"
        class="flex items-center p-2 rounded-lg hover:bg-gray-100" role="menuitem">
        <span class="material-symbols-outlined text-indigo-600 mr-2">calendar_month</span> Jadwal
      </a>
    </div>

    <a href="index.php?page=nilai" class="flex items-center px-3 py-2 text-sm rounded-lg <?= navActive('nilai', $page, $sub) ?>">
