    ...
    <div class="flex items-center gap-3">
      <?php if ($canAdmin): ?>
        ...
        <a href="<?= htmlspecialchars($toggleUrl) ?>" class="px-4 py-2 bg-white/20 text-white rounded-lg hover:bg-white/30">
          <?= $isAll ? 'Tampilkan Punyaa Saya' : 'Tampilkan Semua' ?>
        </a>
      <?php endif; ?>
  ...
  <?php if (count($certificates) > 0): ?>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <?php foreach ($certificates as $cert): ?>
        <div class="bg-gray-800/50 rounded-2xl border border-gray-700/50 hover:border-yellow-500/50 transition-all duration-300 group shadow-md overflow-hidden" data-cert-id="<?= (int)$cert['id'] ?>" data-cert-title="<?= htmlspecialchars($cert['title'], ENT_QUOTES, 'UTF-8') ?>" data-cert-by="<?= htmlspecialchars($cert['issued_by'] ?? '', ENT_QUOTES, 'UTF-8') ?>" data-cert-at="<?= $cert['issued_at'] ? date('d M Y', strtotime($cert['issued_at'])) : '' ?>">
          <!-- Preview -->
          <div class="relative h-52 bg-gray-900/20 flex items-center justify-center overflow-hidden">
            <?php
            ...
            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center pointer-events-none">
              <button onclick="viewCertificate(<?= $cert['id'] ?>)"
                class="px-4 py-2 bg-white/20 backdrop-blur-sm text-white rounded-lg hover:bg-white/30 transition-colors pointer-events-auto">
