  ...
  <section class="px-6 py-6">
    <form method="GET" action="index.php" class="max-w-3xl">
      ...
      <div class="relative flex items-stretch">
        ...
        <input type="text" name="q" value="<?= htmlspecialchars($q) ?>" class="flex-1 pl-11 pr-4 py-2 rounded-l-xl bg-[#313338] border border-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500">
        <select name="cat" class="mx-2 px-3 py-2 rounded bg-[#313338] border border-gray-700 text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500">
    ...
    <?php if (!empty($allNews)): ?>
      ...
      <a href="index.php?page=news_show&id=<?= (int)$first['id'] ?>" class="md:col-span-2 relative rounded-xl overflow-hidden shadow-lg bg-[#313338] border border-gray-800 block group">
        <?php if (!empty($first['thumbnail'])): ?>
          <img src="<?= htmlspecialchars(($baseUrl ? $baseUrl . '/' : '') . ltrim($first['thumbnail'], '/')) ?>" alt="Thumbnail" class="w-full h-64 object-cover opacity-90">
        <?php endif; ?>
