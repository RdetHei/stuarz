        ...
        <tbody class="bg-[#111827] divide-y divide-gray-800">
          <?php if (count($rows) > 0): ?>
            <?php foreach ($rows as $i => $row): ?>
              ...
              <tr class="hover:bg-[#1f2937] transition" data-level="<?= $rowLevel; ?>">
                ...
                <td class="px-6 py-3">
                  <div class="flex justify-end gap-2">
                    <a href="index.php?page=edit_user&id=<?= (int)$row['id']; ?>" class="px-3 py-1 bg-blue-600/80 text-white text-xs rounded-md hover:bg-blue-500 border border-blue-500/30">Edit</a>
                    <a href="index.php?page=delete_user&id=<?= (int)$row['id']; ?>" onclick="return confirm('Yakin ingin menghapus akun ini?')" class="px-3 py-1 bg-red-600/80 text-white text-xs rounded-md hover:bg-red-500 border border-red-500/30">Hapus</a>
                  </div>
                </td>
              </tr>
            <?php endforeach; ?>
