...
<div class="max-w-7xl mx-auto p-6">
  <div class="flex items-center justify-between mb-6">
    ...
    <a href="index.php?page=tasks/create" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg px-4 py-2 font-medium transition">Add Task</a>
  </div>
    ...
    <table class="min-w-full divide-y divide-gray-700">
      <thead class="bg-gray-900">
        <tr>
          ...
          <th class="px-6 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Deadline</th>
      ...
      <tbody class="divide-y divide-gray-700">
        ...
        <tr class="hover:bg-gray-800 group transition">
          <td class="px-6 py-4 whitespace-nowrap text-gray-300"> <?= htmlspecialchars($t['title'] ?? '') ?> </td>
          <td class="px-6 py-4 whitespace-nowrap text-gray-400"> <?= htmlspecialchars($t['subject_name'] ?? '') ?> </td>
          <td class="px-6 py-4 whitespace-nowrap text-gray-400"> <?= htmlspecialchars($t['class_name'] ?? '') ?> </td>
          <td class="px-6 py-4 whitespace-nowrap text-gray-400"> <?= htmlspecialchars($t['deadline'] ?? '') ?> </td>
          <td class="px-6 py-4 whitespace-nowrap">
            <?php if (($t['status'] ?? '') === 'Completed'): ?>
              <span class="inline-block px-2 py-1 text-xs font-semibold rounded bg-green-600 text-white">Completed</span>
            <?php else: ?>
              <span class="inline-block px-2 py-1 text-xs font-semibold rounded bg-yellow-500 text-gray-900">Pending</span>
            <?php endif; ?>
          </td>
          <td class="px-6 py-4 whitespace-nowrap text-right">
            <a href="index.php?page=tasks/edit&id=<?= $t['id'] ?>" class="inline-flex items-center text-indigo-400 hover:text-indigo-200 mr-2" title="Edit"><svg xmlns='http://www.w3.org/2000/svg' class='lucide lucide-pencil' width='18' height='18' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M15.232 5.232l-1.464-1.464a2 2 0 0 0-2.828 0l-6.536 6.536a2 2 0 0 0-.586 1.414V15h3.182a2 2 0 0 0 1.414-.586l6.536-6.536a2 2 0 0 0 0-2.828z'/><path d='M13.5 6.5l-7 7'/></svg></a>
