...
class users
{
    ...
    {
        $rows = [];
        $sql = "SELECT id, username, email, `level`, join_date, avatar, banner FROM users ORDER BY id DESC";
    ...
    {
        $sql = "SELECT id, username, email, `level`, join_date, avatar, banner FROM users WHERE id = ? LIMIT 1";
    ...
    {
        // ensure join_date is set at creation time
        $sql = "INSERT INTO users (username, email, password,`level`, avatar, banner, join_date) VALUES (?, ?, ?, ?, ?, ?, NOW())";
        $stmt = mysqli_prepare($this->conn, $sql);
